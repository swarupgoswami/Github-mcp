...
server.tool("createRepo", "creates a new repository on GitHub", {
...
server.tool("getIssues", "get the open issues from a github repository", {
    ...
    const issuesText = issues.map((issue) => `#${issue.number} - ${issue.title}\n${issue.url}`).join('\n\n');
    return {
        content: [
            {
                type: "text",
...
server.tool("listRepo", "list all the repo of a github user", {
